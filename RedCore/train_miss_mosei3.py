...
from loadmosei import load_mosei
...
def binary_metrics(y_pred, y_true, exclude_zero=False):
    ...
    return [non_zeros_acc2, non_zeros_f1_score, acc2, f_score, binary_truth, binary_preds]




def eval(model, val_iter, is_save=False, phase='test'):
    datasize = len(val_iter) 
    #batch_size = 128
    #mp = missing_pattern(3, dataset_size * batch_size, [0.1, 0.1, 0.1])
    model.eval()
    total_pred = []
    total_label = []
    total_miss_type = []
    ...
    binary_results = binary_metrics(total_pred, total_label)
    total_label = binary_results[4]
    total_pred = binary_results[5]
    ...
    return acc, uar, f1, cm, binary_results
...
if __name__ == '__main__':
    #rate_list = [[0, 0, 0], [0.1, 0.1, 0.1], [0.2, 0.2, 0.2], [0.3, 0.3, 0.3], [0.4, 0.4, 0.4], [0.5, 0.5, 0.5], [0.6, 0.6, 0.6], [0.7, 0.7, 0.7], [0.8, 0.8, 0.8], [0.9, 0.9, 0.9]]
    rate_list = [ [0.8, 0.5, 0.5], [0.5, 0.8, 0.5], [0.5, 0.5, 0.8], [0.2, 0.5, 0.8], [0.5, 0.2, 0.8], [0.8, 0.5, 0.2]]
    ...
    result_dir = os.path.join(opt.log_dir, opt.name, 'results'+ str(opt.rate_indx))
    ...
    if opt.has_test:                                    # create a dataset given opt.dataset_mode and other options
        ...
        #dataset, val_dataset, tst_dataset = load_mosei()
    else:
    ...
    #mp = missing_pattern(3, dataset_size * opt.batch_size, [0.5, 0.5, 0.5])
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            #upper_bound = min(opt.batch_size * (i+1), data_size)
            #data['missing_index'] = mp[opt.batch_size * i : upper_bound, :]
            model.set_input(data)           # unpack data from dataset and apply preprocessing
        ...
        # eval
        acc, uar, f1, cm, _ = eval(model, val_dataset)
        ...
        if opt.has_test and opt.verbose:
            acc, uar, f1, cm, br = eval(model, tst_dataset)
            acc_b, f1_b = br[2], br[3]
            logger.info('Tst result of epoch %d acc %.4f uar %.4f f1 %.4f' % (epoch, acc, uar, f1))
            logger.info('\n{}'.format(cm))
            logger.info('Tst result of epoch %d acc_b %.4f f1_b %.4f' % (epoch, acc_b, f1_b))
            logger.info('\n{}'.format(cm))

        # record epoch with best result
        if opt.corpus_name == 'IEMOCAP' or 'CMU_MOSEI':
    ...
    if opt.has_test:
        ...
        acc, uar, f1, cm, binary_metrics= eval(model, tst_dataset, is_save=True, phase='test')
        logger.info('Tst result acc %.4f uar %.4f f1 %.4f' % (acc, uar, f1))
        logger.info('Tst result non_zeros_acc2 %.4f non_zeros_f1_score %.4f acc2 %.4f f_score %.4f' % (binary_metrics[0], binary_metrics[1], binary_metrics[2], binary_metrics[3]))   #non_zeros_acc2, non_zeros_f1_score, acc2, f_score
        logger.info('\n{}'.format(cm))
        recorder_lookup['total'].write_result_to_tsv({
            'acc': binary_metrics[2],
            'uar': binary_metrics[3],
            'f1': binary_metrics[3],
        }, cvNo=opt.cvNo)
    else:
        recorder_lookup['total'].write_result_to_tsv({
